...
public class FragmentAddEdit extends Fragment {
	...
	public View onCreateView(
		...
		if(args != null) {
			...
			} else editMode = FragmentEditMode.Add;
		} else {
			task     = null;
		...
		save.setOnClickListener(view -> {
			int sortOrd = 0;
			if(sortOrder.length() > 0) sortOrd = Integer.parseInt(sortOrder.getText().toString());

			String        error        = null;
			String        nName        = name.getText().toString();
			String        nDescription = description.getText().toString();
			AppDb         appDb        = AppDb.getInstance(getContext());
			ContentValues values       = new ContentValues();
			if(editMode == FragmentEditMode.Edit && task != null) {
				if(!Objects.equals(nName, task.getName()) && nName.length() > 0) values.put(Tasks.Name, nName);
				if(!Objects.equals(nDescription, task.getDescription()) &&
				   description.length() > 0) {
					values.put(Tasks.Description, nDescription);
				}
				if(sortOrd != task.getSortOrder()) values.put(Tasks.SortOrder, sortOrd);
				if(values.size() != 0) appDb.updateTasks(task.getId(), values, null, null);
				else error = "No values have been changed";
			} else if(editMode == FragmentEditMode.Add) {
				if(nName.length() > 0) {
					values.put(Tasks.Name, nName);
					values.put(Tasks.Description, nDescription);
					values.put(Tasks.SortOrder, sortOrd);
					appDb.insertTask(values);
				} else error = "Enter a name";
			}
			if(error == null) {
				if(saveClickListener != null) saveClickListener.onFragAddEditSaveClicked();
			} else {
